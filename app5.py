...
def process_text(original_text):
    ...
    try:
        ...
        spacy_tags = [(token.text, token.lemma_, token.pos_, token.tag_, token.idx) for token in doc_spacy if token.is_alpha]
...
def create_dataframe(processed_data):
    ...
    try:
        ...
        for token_text, lemma, pos, tag, idx in spacy_tags:
...
def parse_childes_file(file_rel, speaker_option="both"):
    tag_map = {
        "child":        ["*CHI:"],
        "investigator": ["*MOT:", "*FAT", "*INV:"],
        "both":         ["*CHI:", "*MOT:", "*FAT", "*INV:"]
    }
    target_tags = tag_map.get(speaker_option, tag_map["both"])
    utterances  = {"CHI": [], "MOT": [], "FAT": [], "INV": []}
    sequence   = []
...
def download_analysis():
    ...
    # ---------- Summary sheet ----------
    summary_rows = []
    # print(f'analysis cache dictionary is {analysis_cache}')
    for file_rel, a in analysis_cache.items():
        ...
        for spk in ("CHI", "MOT", "FAT",  "INV"):
            stats = a.get("speakers", {}).get(spk, {})
            row[f"{spk}_Result"] = stats.get("result_count", 0)
            row[f"{spk}_Result_breakdown"] = stats.get("result_breakdown", {})
            row[f"{spk}_Manner"] = stats.get("manner_count", 0)
            row[f"{spk}_Manner_breakdown"] = stats.get("manner_breakdown", {})
